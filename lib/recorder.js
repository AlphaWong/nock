...
function generateRequestAndResponse(
  ...
  return [
    '\n',
    `nock('${getScope(options)}', ${JSON.stringify({
      encodedQueryParams: true,
    })})\n`,
    `  .${getMethod(options).toLowerCase()}('${path}'`,
    requestBody ? `, ${JSON.stringify(requestBody)}` : '',
    ')\n',
  ]
    .concat(
      Object.entries(reqheaders || {}).map(
        ([fieldName, fieldValue]) =>
          `  .matchHeader(${JSON.stringify(fieldName)}, ${JSON.stringify(
            fieldValue
          )})\n`
      )
    )
    .concat([
      queryIndex !== -1 ? `  .query(${JSON.stringify(encodedQueryObj)})\n` : '',
      `  .reply(${res.statusCode.toString()}, ${JSON.stringify(
        responseBody
      )}, ${inspect(res.rawHeaders)});\n`,
    ])
    .join('')
}
