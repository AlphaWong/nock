...
function generateRequestAndResponse(
  ...
  Object.entries(reqheaders || {}).forEach(([rawFieldName, rawFieldValue]) => {
    const fieldName = JSON.stringify(rawFieldName)
    const fieldValue = JSON.stringify(rawFieldValue)
    lines.push(`  .matchHeader(${fieldName}, ${fieldValue})`)
  })
